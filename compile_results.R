...
for(i in 1:length(set_files)){
      ...
      # This is a fix around having multiple runs
      if(nrow(est_raw)> 4){
      	est_raw <- est_raw[(nrow(est_raw) - 3):nrow(est_raw), ]
	  #    print(nrow(est_raw))
	  }
	if(nrow(est_raw) != 4){
	print(paste0('error in ', row_name_temp))
	  next
	}
      ...
      if(length(grep('true_dat*xml', est_raw[, 1])) > 1){
