...
t1 <- get_tree_cal(span_cut = c(1.05, 1.2), max_cal = max_cal, min_cal = min_cal, tr_time = 100, n_tax = 50, print_trees = T)
p1 <- t1$chronogram
...
cat(paste(cal_time, mean_rate, sd_rate, unlist(run_1), slope_true, r_true, collapse = ' '), sep = '\n', file = 'SET1.txt')
