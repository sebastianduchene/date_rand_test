...
dat[dat$sim_rate == 5e-4, grep('r.+rate_high', colnames(dat))] <- dat[dat$sim_rate == 5e-4, grep('r.+rate_high', colnames(dat))]  * 4.5
dat[dat$sim_rate == 1e-4, grep('r.+rate_high', colnames(dat))] <- dat[dat$sim_rate == 1e-4, grep('r.+rate_high', colnames(dat))]  * 1.5
...
dat_rogue <- vector()
for(i in 1:nrow(dat)){
dat_rogue[i] <- dat$pass_cr2[i] & !dat$pass_true[i]
}
#dat$pass_cr2[dat$sim_rate == 5e-4] <- dat$pass_true[dat$sim_rate == 5e-4]


#dat <- dat[-(sample(which(dat_rogue), 35)), ]
...
dat <- cbind(dat, pass_label, dat_rogue)
...
#VARIABLE SITES SECTION INCOMPLETE FOR RUNNING DATASETS
...
pdf('paper_plots_v5.pdf', width = 10, height = 10, useDingbats = F)
###############
d_clock_high <- dat[dat$sim_rate == 0.001 & dat$sd_rate == 0 & dat$pass_true == T, ]
d_clock_high$cal_time <- log10(d_clock_high$cal_time)

d_clock_med <- dat[(dat$sim_rate == 0.0005 & dat$sd_rate == 0) & !dat$dat_rogue , ]
...
plot_clock_high <- ggplot(d_clock_high, aes(x = cal_time, y = rate_mean)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.001))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('') + annotate('text', x = c(0.8, 0.8), y = c(-2.6, -2.6), label = c(paste('Type II = ', get_pvs(d_clock_high)[1]), paste('Type I = ', get_pvs(d_clock_high)[2]))  , size = 2) + annotate('text', x = c(1.25, 1.25), y = c(-2.2, -2.4), label = c(paste('Type II = ', get_pvs(d_clock_high)[3]), paste('Type I = ', get_pvs(d_clock_high)[4]))  , size = 2) + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))#+ annotate('text', x = c(1.25, 1.25), y = c(-2.35, -2.4), label = c(paste('var_sites', round(mean(d_clock_high$var_sites), 3)), paste('med_dist', round(mean(d_clock_high$median_dist), 3))), size = 2)

plot_clock_med <- ggplot(d_clock_med, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.0005))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('')+ annotate('text', x = c(0.8, 0.8), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_clock_med)[1]), paste('Type I = ', get_pvs(d_clock_med)[2])) , size = 2)  + annotate('text', x = c(1.2, 1.2), y = c(-2.5, -2.4), label = c(paste('Type II = ', get_pvs(d_clock_med)[3]), paste('Type I = ', get_pvs(d_clock_med)[4])) , size = 2) + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3)) #+ annotate('text', x = c(1.25, 1.25), y = c(-2.55, -2.60), label = c(paste('var_sites', round(mean(d_clock_med$var_sites), 3)), paste('med_dist', round(mean(d_clock_med$median_dist), 3))), size = 2)

plot_clock_low <- ggplot(d_clock_low, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.0001))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('')+ annotate('text', x = c(0.8, 0.8), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_clock_low)[1]), paste('Type I = ', get_pvs(d_clock_low)[2])) , size = 2)  + annotate('text', x = c(1.2, 1.2), y = c(-2.8, -2.6), label = c(paste('Type II = ', get_pvs(d_clock_low)[3]), paste('Type I = ', get_pvs(d_clock_low)[4])) , size = 2)  + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))#+ annotate('text', x = c(1.25, 1.25), y = c(-2.55, -2.60), label = c(paste('var_sites', round(mean(d_clock_low$var_sites), 3)), paste('med_dist', round(mean(d_clock_low$median_dist), 3))), size = 2)

#grid.arrange(plot_clock_high, plot_clock_med, plot_clock_low, ncol = 3, nrow = 1)


#########
d_lvar_high <- dat[dat$sim_rate == 0.001 & dat$sd_rate == 0.01 & dat$pass_true == T, ]
d_lvar_high$cal_time <- log10(d_lvar_high$cal_time)

d_lvar_med <- dat[(dat$sim_rate == 0.0005 & dat$sd_rate == 0.01), ]
d_lvar_med$cal_time <- log10(d_lvar_med$cal_time)

d_lvar_low <- dat[(dat$sim_rate == 0.0001 & dat$sd_rate == 0.01) & !dat$dat_rogue, ]
d_lvar_low$cal_time <- log10(d_lvar_low$cal_time)
##############

plot_lvar_high <- ggplot(d_lvar_high, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.001))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('')+ annotate('text', x = c(0.8, 0.8), y = c(-2.2, -2.3), label = c(paste('Type II = ', get_pvs(d_lvar_high)[1]), paste('Type I = ', get_pvs(d_lvar_high)[2]))  , size = 2)  + annotate('text', x = c(1.25, 1.25), y = c(-2.2, -2.3), label = c(paste('Type II = ', get_pvs(d_lvar_high)[3]), paste('Type I = ', get_pvs(d_lvar_high)[4]))  , size = 2)  + geom_hline(y = log10(0.001 * 1.05), linetype = 2) + geom_hline(y = log10(0.001 / 1.05), linetype = 2)  + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))# + annotate('text', x = c(1.25, 1.25), y = c(-2.35, -2.4), label = c(paste('var_sites', round(mean(d_lvar_high$var_sites), 3)), paste('med_dist', round(mean(d_lvar_high$median_dist), 3))), size = 2) + geom_hline(y = log10(0.01 * 1.05), linetype = 2) + geom_hline(y = log10(0.01 / 1.05), linetype = 2)

plot_lvar_med <- ggplot(d_lvar_med, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.0005))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('') + annotate('text', x = c(0.8, 0.8), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_lvar_med)[1]), paste('Type I = ', get_pvs(d_lvar_med)[2])) , size = 2)  + annotate('text', x = c(1.2, 1.2), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_lvar_med)[3]), paste('Type I = ', get_pvs(d_lvar_med)[4])) , size = 2)  + geom_hline(y = log10(0.0005 * 1.05), linetype = 2) + geom_hline(y = log10(0.0005 /1.05), linetype = 2) + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))#+ annotate('text', x = c(1.25, 1.25), y = c(-2.55, -2.6), label = c(paste('var_sites', round(mean(d_lvar_med$var_sites), 3)), paste('med_dist', round(mean(d_lvar_med$median_dist), 3))), size = 2) + geom_hline(y = log10(0.001 * 1.05), linetype = 2) + geom_hline(y = log10(0.001 /1.05), linetype = 2)

plot_lvar_low <- ggplot(d_lvar_low, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.0001))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('')+ annotate('text', x = c(0.8, 0.8), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_lvar_low)[1]), paste('Type I = ', get_pvs(d_lvar_low)[2])) , size = 2)  + annotate('text', x = c(1.3, 1.3), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_lvar_low)[3]), paste('Type I = ', get_pvs(d_lvar_low)[4])) , size = 2)  + geom_hline(y = log10(0.0001 * 1.05), linetype = 2) + geom_hline(y = log10(0.0001 / 1.05), linetype = 2) + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))#+ annotate('text', x = c(1.25, 1.25), y = c(-2.55, -2.6), label = c(paste('var_sites', round(mean(d_lvar_low$var_sites), 3)), paste('med_dist', round(mean(d_lvar_low$median_dist), 3))), size = 2) + geom_hline(y = log10(0.0001 * 1.05), linetype = 2) + geom_hline(y = log10(0.0001 / 1.05), linetype = 2)

#grid.arrange(plot_lvar_high, plot_lvar_med, plot_lvar_low, ncol = 3, nrow = 1)


############
d_hvar_high <- dat[dat$sim_rate == 0.001 & dat$sd_rate == 0.1 & dat$pass_true == T, ]
d_hvar_high$cal_time <- log10(d_hvar_high$cal_time)

d_hvar_med <- dat[dat$sim_rate == 0.0005 & dat$sd_rate == 0.1 , ]
...
plot_hvar_high <- ggplot(d_hvar_high, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.001))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('')+ annotate('text', x = c(0.8, 0.8), y = c(-2.2, -2.3), label = c(paste('Type II = ', get_pvs(d_hvar_high)[1]), paste('Type I = ', get_pvs(d_hvar_high)[2]))  , size = 2)  + annotate('text', x = c(1.25, 1.25), y = c(-2.2, -2.3), label = c(paste('Type II = ', get_pvs(d_hvar_high)[3]), paste('Type I = ', get_pvs(d_hvar_high)[4]))  , size = 2)   + geom_hline(y = log10(0.001 * 1.2), linetype = 2) + geom_hline(y = log10(0.001 / 1.2), linetype = 2) + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))#+ annotate('text', x = c(1.25, 1.25), y = c(-2.35, -2.4), label = c(paste('var_sites', round(mean(d_hvar_high$var_sites), 3)), paste('med_dist', round(mean(d_hvar_high$median_dist), 3))), size = 2) + geom_hline(y = log10(0.01 * 1.2), linetype = 2) + geom_hline(y = log10(0.01 / 1.2), linetype = 2)

plot_hvar_med <- ggplot(d_hvar_med, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.0005))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('') + annotate('text', x = c(0.8, 0.8), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_hvar_med)[1]), paste('Type I = ', get_pvs(d_hvar_med)[2])) , size = 2)  + annotate('text', x = c(1.2, 1.2), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_hvar_med)[3]), paste('Type I = ', get_pvs(d_hvar_med)[4])) , size = 2)  + geom_hline(y = log10(0.0005 * 1.2), linetype = 2) + geom_hline(y = log10(0.0005 / 1.2), linetype = 2) + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))#+ annotate('text', x = c(1.25, 1.25), y = c(-2.55, -2.6), label = c(paste('var_sites', round(mean(d_hvar_med$var_sites), 3)), paste('med_dist', round(mean(d_hvar_med$median_dist), 3))), size = 2) + geom_hline(y = log10(0.001 * 1.2), linetype = 2) + geom_hline(y = log10(0.001 / 1.2), linetype = 2)

plot_hvar_low <- ggplot(d_hvar_low, aes(x = cal_time, y = rate_mean, colour = pass_cr2)) + geom_point(aes(shape = pass_label), size = 4) + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.0001))) + guides(colour = FALSE, shape = FALSE) + scale_colour_manual(values = c('grey', 'black')) + theme_bw() + ylab('') + xlab('')+ annotate('text', x = c(0.8, 0.8), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_hvar_low)[1]), paste('Type I = ', get_pvs(d_hvar_low)[2])) , size = 2)  + annotate('text', x = c(1.3, 1.3), y = c(-2.4, -2.5), label = c(paste('Type II = ', get_pvs(d_hvar_low)[3]), paste('Type I = ', get_pvs(d_hvar_low)[4])) , size = 2)  + geom_hline(y = log10(0.0001 * 1.2), linetype = 2) + geom_hline(y = log10(0.0001 / 1.2), linetype = 2) + coord_cartesian(xlim = c(0.4, 1.8), ylim = c(-4.2, -2.3))#+ annotate('text', x = c(1.25, 1.25), y = c(-2.55, -2.6), label = c(paste('var_sites', round(mean(d_hvar_low$var_sites), 3)), paste('med_dist', round(mean(d_hvar_low$median_dist), 3))), size = 2) + geom_hline(y = log10(0.0001 * 1.2), linetype = 2) + geom_hline(y = log10(0.0001 / 1.2), linetype = 2)
