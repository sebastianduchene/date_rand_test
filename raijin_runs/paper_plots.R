...
plot_clock_low_cr1 <- ggplot(d_clock_low, aes(x = cal_time, y = rate_mean, colour = pass_cr1)) + geom_point() + geom_errorbar(aes(ymin = rate_low, ymax = rate_high)) + geom_hline(aes(yintercept = log10(0.0001))) + guides(colour = FALSE) + scale_colour_manual(values = c('red', 'black')) + theme_bw() + xlab(expression(paste(Log[10], ' Calibration', ' (year)' ))) + ylab('')  + annotate('text', x = c(1.2, 1.2), y = c(-3, -3.2), label = c(paste('PPV = ', get_pvs(d_clock_low)[1]), paste('NPV = ', get_pvs(d_clock_low)[2])) , size = 2) 
