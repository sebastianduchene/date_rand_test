...
# IN THIS CASE WE WILL GET THE ROUGE DATA

rouge_data <- vector()
for(i in 1:nrow(dat)){
      rouge_data[i] <- dat$pass_cr2[i] & (!dat$pass_true[i])
}


plot(log10(dat$cal_time[dat$sim_rate == 1e-3]), dat$var_sites[dat$sim_rate == 1e-3], col= c(1, 2)[rouge_data[dat$sim_rate == 1e-3] + 1], pch = (dat$pass_cr2+1)[dat$sim_rate == 1e-3])


stop('testing rouge data')
